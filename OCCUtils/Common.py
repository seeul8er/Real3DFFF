...
def get_boundingbox(shape, tol=TOLERANCE, use_mesh=False, tess_tol=1e-3):
    """
    Returns the bounding box coordinates of a SMESH or a TopoDS_Shape
    Modified version of the original code by Wolfgang Christl

    :param shape: TopoDS_Shape such as TopoDS_Face OR a SMESH Mesh
    :param tol: tolerance
    :param use_mesh: Create triangulated mesh first, slower but more exact!
    :param tess_tol: Triangulation linear deflection. If to high it might not generate a geometry
    :return: xmin, ymin, zmin, xmax, ymax, zmax
    """
    from OCC.SMESH import SMESH_Mesh
    if isinstance(shape, TopoDS_Shape):
        if use_mesh:
            # Tesselate first. Otherwise it may be shit result of BB
            m = BRepMesh_IncrementalMesh()
            m.SetParallel(True)
            m.SetDeflection(tess_tol)
            m.SetShape(shape)
            m.Perform()
        bbox = Bnd_Box()
        # bbox.SetGap(tol)
        ...
        return xmin, ymin, zmin, xmax, ymax, zmax
    elif isinstance(shape, SMESH_Mesh):
        xmin = 0
        ymin = 0
        zmin = 0
        xmax = 0
